...
- Deploys happen only on push to `main` (DocoCD webhook). Router additions are inert until the tunnel sends bare-hostname traffic to Traefik, so PR 2 can merge and deploy safely before any Cloudflare change.
...
Run: `ssh server "sudo grep -o 'dynabase.nl' /home/diego/services_data/networking/traefik/acme.json | sort -u"`
Expected: output includes `dynabase.nl` (wildcard cert issued). If empty after ~2 min, check `ssh server "docker logs traefik 2>&1 | grep -i acme | tail -20"` for DNS-01 errors.
...
> All routers added here are inert until the tunnel is repointed at Traefik (the manual cutover). They can merge and deploy with zero effect on current behavior.
...
### Task 5: Add Traefik routers to the 3 services that bypass Traefik today
...
**Interfaces:**
- Consumes: the `*.dynabase.nl` wildcard cert from Task 1.
- Produces: routers `grist` (grist.dynabase.nl → grist:9999), `mattermost` (mattermost.dynabase.nl → mattermost:8065), `authelia` (auth.dynabase.nl → authelia:9091).

These containers currently have **no** Traefik labels (the tunnel hit them directly). Add a full `labels:` block. Each container has one Traefik service, so no explicit `service=` is needed. Router names have no `-external` suffix because these are public-only (no `.local` counterpart), matching the existing `doco-cd` pattern.

- [ ] **Step 1: Confirm insertion points**

Run: `grep -n "container_name: grist" services/tools/docker-compose.yaml; grep -n "container_name: mattermost" services/collaboration/docker-compose.yaml; grep -n "container_name: authelia" services/security/docker-compose.yaml`
Add each `labels:` block inside the respective service definition (sibling to `container_name`/`image`/`environment`).
...
- [ ] **Step 3: mattermost labels** — add to the `mattermost` service in `services/collaboration/docker-compose.yaml`:
...
- [ ] **Step 4: authelia labels** — add to the `authelia` service in `services/security/docker-compose.yaml` (note host is `auth`, not `authelia`):
...
Run: `uv run yamllint services/tools/docker-compose.yaml services/collaboration/docker-compose.yaml services/security/docker-compose.yaml`
Expected: no errors.

- [ ] **Step 6: Commit**

```bash
git add services/tools/docker-compose.yaml services/collaboration/docker-compose.yaml services/security/docker-compose.yaml
git commit -m "Services|Add: Traefik routers for grist, mattermost, authelia (route via Traefik)"
```
...
```bash
git push
gh pr create --draft --title "Services: wildcard Cloudflare tunnel — per-service public routers" --body "Adds bare-hostname Traefik routers for the 13 public services (n8n and doco-cd already had theirs). Inert until the manual tunnel cutover in the runbook. Depends on PR 1 (wildcard cert). Spec/plan: docs/superpowers/"
```
...
> Prerequisite: PR 1 + PR 2 merged and deployed; Task 1 verification confirms the `*.dynabase.nl` cert is issued. This is dashboard work (remotely-managed tunnel), tracked here, not code.
...
Expected: 200/302 and a valid edge cert. Exercise the app briefly (login, a page load). Watch the known-tricky ones: **nextcloud** (trusted_domains / overwrite-host redirects), **immich** (large uploads / websockets), **mattermost** (websockets), **home_assistant** (trusted_proxies).
...
- Routers for the 5 bypassers: grist/mattermost/authelia → Task 5; home_assistant → Task 6; doco-cd already has one (documented in inventory, repointed in A5). ✓
...
- PR split (foundation vs routers) → PR 1 / PR 2. ✓
