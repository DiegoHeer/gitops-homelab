...
| **Tools** | IT-Tools, BentoPDF, Docuseal, Changedetection, Tandoor + PostgreSQL, Speedtest Tracker, ConvertX |
...
### Taking a service out of service

To stop a service without deleting its definition, scale it to zero:
...
DocoCD keeps reconciling the stack, so the container is removed on the next deploy and stays gone until the line does. Runtime state under `/home/diego/services_data/` and named volumes survive untouched; the image may be pruned once nothing uses it, so re-enabling re-pulls it. Scaling *every* service in a stack to zero takes the whole stack down — its network and volumes still exist.

`webhook_filter` is a different tool for a different job. Pointing it at a ref that never exists (`^refs/heads/__paused__$`) makes DocoCD skip the stack while its containers keep running — useful to hold config drift back, useless if the goal is for something to stop. See [ADR 0033](docs/adr/0033-scale-zero-to-take-services-out-of-service.md).
